...
public interface LobbyRepository extends CrudRepository<Lobby, Long>{
    ...
    @Query(value = "select * From Lobby where type = 'Public' and player_count = 1 and game_lobby ='CHECKER' and status = 'Online' ORDER BY id DESC limit ?1 ;"
    , nativeQuery = true)
    List<Lobby> findCheckersLobbysAvalible(int counter);
}
